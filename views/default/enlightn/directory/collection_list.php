        ...
        <?php
        		...
        		if (is_array($vars['collections'])) {
					foreach ($vars['collections'] as $key=>$collection) {
						...
						echo '<li id="area' . $collection->id . '" class="dropable" data-listId="' . $collection->id . '" data-listName="' . $collection->name . '"><a class="cat" href="/list' . $collection->id . '"><span class="count">' . $count_member . '</span>' . $collection->name . '<span class="ico ' . ('0'==$collection->owner_guid?'public':'private') . '-ico"></span></a></li>';
					}
        		}
        ?>
