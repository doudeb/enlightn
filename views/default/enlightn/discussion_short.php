...
<script>
$("#discussion_link_<?php echo $post->id ?>").click( function () {
	loadContent('#discussion','<?php echo $vars['url'] ?>/mod/enlightn/ajax/discussion.php?discussion_id=<?php echo $vars['entity']->guid; ?>','load',"#discussion_post<?php echo $post->id; ?>");
	return false;
});
</script>
...
<div id="<?php echo false===$flag_readed?'short_post_view_unread':'short_post_view'?>"><!-- start the topic_post -->
	<a href="" id="discussion_link_<?php echo $post->id ?>"><?php echo search_get_highlighted_relevant_substrings($vars['entity']->title,$vars['query']); ?></a>
